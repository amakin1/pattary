...
def pt_get_shop_product(url_shop_site):
    ...
    pd_count+=n
    print '=== product num shop %s %d ,all shop %d==='%(shop_name,n,pd_count)
    
def pt_get_prod_detail(url):
    global pd_count_has_get
    pd_count_has_get+=1
    pd_dict = {}
    print '--- get prod detail %d of %d---'%(pd_count_has_get,pd_count)
...
def pt_save_shop_pd_excel_init():
    global pd_x,pd_y,sheet_pd,sheet_shop,style,style1,wkb
    pt_time_init()
    pd_x = shop_x = 2
    pd_y = shop_y = 1
    pattern = xlwt.Pattern() # Create the Pattern
        ...
        if True:
            for k,v in shop_info_dict.iteritems():
                ...
                if isinstance(v,dict):
                    ...
                    pt_get_shop_product(shop_site) #gen per shop_pd_dict
                    if True:
                        ...
                        #'''
                            if isinstance(v,list):
                                for x in xrange(len(v)):
                                    ...
                                    id = v[x]
                                    print '\n'
                                    pt_get_prod_detail(id) #gen per pd detail
                                    pt_save_shop_pd_func(shop_pd_detail_dict) #save per pd detail
                                    nn+=1
